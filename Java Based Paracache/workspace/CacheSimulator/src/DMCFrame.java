...
class DMCFrame extends JFrame {
	...
	private JLabel lCacheHits, lCacheMisses, lProgress, lBits1, lBits2, tProgress;
	...
	//CONSTRUCTOR
	public DMCFrame(int cacheSizeBit, int memorySizeBit, int wordSizeBit){

		//SET PROPERTIES OF THE MAIN FRAME
		setTitle("Direct Mapped Cache");
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setIconImage(Toolkit.getDefaultToolkit().createImage(DMCFrame.class.getResource("camera.jpg")));
	    ...
	    tagDigit=memorySizeBit-cacheSizeBit-wordDigit;
	    blockDigit=cacheSizeBit;
		...
		cacheHitsMisses.setOpaque(true);
		cacheHitsMisses.setBackground(new Color(255,255,255));
		...
		pBitsInMM.setBackground(Color.WHITE);
	...
	*/
	public void reStart(){
		...
		tProgress.setText("<html> Let's start over. <br> Please generate the Address Reference String.</html>");
	...
	public void step(){
		...
		if (moveStatus == 0){
			back.setEnabled(false);
			tProgress.setText("<html> You cannot go back any further. Please click on \"Next\" or \"Restart\" to continue.");
				//tProgress.setCaretPosition(0);
		...
		while (tempState <= moveStatus){
			...
			case 1: //IF A NEW CYCLE IS BEGINNING, OBTAIN NEXT ADDRESS REFERENCE
				...
				//ENSURE THAT binAddress is align to the total length 
				
				if (binAddress.length() < tagDigit+blockDigit+wordDigit){
					int zeroes = tagDigit+blockDigit+wordDigit - binAddress.length();
					for (int i = 0; i < zeroes; i++)
						binAddress = '0'+binAddress;
				}

				tag = binAddress.substring(0, tagDigit);
				block = binAddress.substring(tagDigit, blockDigit+tagDigit);
				word = binAddress.substring(blockDigit+tagDigit);
				blockMem = binAddress.substring(0, blockDigit+tagDigit);
			...
			case 2: //EVALUATE THE BITS IN MAIN MEMORY ADDRESS AND HIGHLIGHT THEM		 
				...
				if (tempState == moveStatus){
					tProgress.setText("<html>" +hexAddress +" [" + binAddress+ "] = " 
									  + tag + " --> Tag,  "+ block + " --> Block,  " + word + " --> Word."
									  +"<br>The leftmost "+Integer.toString(tagDigit+blockDigit)
									  + " bits, "+tag+block+", and the rightmost "+Integer.toString(wordDigit)
									  + " bits, "+word
									  +", indicate the actual memory block and word in question, respectively.</html>");
			...
			case 3: //EVALUATE THE CACHE BLOCK IN QUESTION AND HIGHLIGHT IT
				...
				//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
				if (tempState == moveStatus){
					tProgress.setText("<html> The bits in the Main Memory Address indicating the cache block are " 
									  + block + ".<br> The equivalent block number in Hexadecimal is "+ Integer.toHexString(intBlockDec) 
									  + ", so the block <b>"+Integer.toHexString(intBlockDec)+"</b> is searched in the cache.</html>");
					//tProgress.setCaretPosition(0);
				}//END IF
				...
				if (cachePanel.stringBlocks[intBlockDec].equals("")){
					...
					if (tempState == moveStatus){
						String current = tProgress.getText() + " <br> Since the required cache block is empty, there is a cache miss.";
						tProgress.setText(current);
						//tProgress.setCaretPosition(0);
					}
				}
					...
					if (tempState == moveStatus){
						...
						String current = tProgress.getText() + "<br>Since the required cache block contains the memory block, there is a cache hit.";
						tProgress.setText(current);
						//tProgress.setCaretPosition(0);
					}
				}
					...
					if (tempState == moveStatus){
						...
						//tProgress.setCaretPosition(0);	
						String current = tProgress.getText() + "<html><br>Since the required cache block does not contain the memory block, there is a cache miss.";
						tProgress.setText(current);
					}
				}
			...
			case 4: //EVALUATE THE MEMORY BLOCK IN QUESTION AND HIGHLIGHT IT
				...
				*/
				...
				if (cachePanel.stringBlocks[intBlockDec].equals("")){
					...
					//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
					if (tempState == moveStatus){
						tProgress.setText("<html>Since the "+Integer.toString(tagDigit+blockDigit)
										  + " leftmost bits are " + tag + block + " [" +Integer.toHexString(intBlockDecMem)+"] "
										  +", the memory block we need (in Hexadecimal) is "+ Integer.toHexString(intBlockDecMem)+"."
										  +"<br>As we saw in the previous step, the required block of the cache is empty "
				...
				//IF MEMORY BLOCK IS ALREADY IN CACHE THEN DO NOTHING	
					...
					if (tempState == moveStatus){
						tProgress.setText("<html>Since the "+Integer.toString(tagDigit+blockDigit)
										  + " leftmost bits are "+tag+block
										  +", the memory block we need (in Hexadecimal) is "+Integer.toHexString(intBlockDecMem)+"."
										  +"<br>As saw in the previous step, "
				...
				//IF THE CACHE BLOCK IS FILLED WITH ANOTHER MEMORY BLOCK
					...
					if (tempState == moveStatus){
						tProgress.setText("<html>Since the " + Integer.toString(tagDigit+blockDigit)
										  + " leftmost bits are "+tag+block
										  +", the memory block we need (in Hexadecimal) is "+ Integer.toHexString(intBlockDecMem) +"."
										  +"<br>The required block of the cache is filled, "
										  +"but with another tag."
										  +"<br> So we need to bring in the block from memory and "
			...
			case 5: //HIGHLIGHT THE CACHE BLOCK WITH THE MEMORY BLOCK NOW IN IT
				...
				//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
				if (tempState == moveStatus){
					tProgress.setText("<html>1. The cache block has a tag associated with it and the tag, "
									  +"as specified by the Tag bit in the Main Memory Address, has a value of "+tag
									  +".<br>2. The word bits, as specified in the Main Memory Address, are "+word
									  +" which indicate the word at offset "+wordDec+" in the block."
									  +"<br>3.If the cache was originally empty or contained a memory block other than the one"
									  +" we required, the count of Cache Misses was incremented."
									  +"<br>If the cache already had the required memory block in it, "

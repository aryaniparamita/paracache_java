...
class VMFrame extends JFrame {
	...
	private JPanel pStringListMM, pAddRefStr, pAutoSelfGen, pBits, pBitsInVM, pBitsInPM;
	...
	private JScrollPane progressScroll, addRefStrScroll, physMemPanelScroll;
	...
	private boolean[] statusFrameEmpty = new boolean[Constant.MAXSIZE];
	private int[] statusFrameLRU = new int[Constant.MAXSIZE];
	...
	private boolean[] statusTLBRowEmpty = new boolean[Constant.MAXSIZE];
	private int[] statusTLBRowLRU = new int[Constant.MAXSIZE];
	...
	private int physicalMemoryBit, tLB, virtualMemory;
	private int physicalMemorySize, tlBSize, virtualMemorySize;
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////

	//CONSTRUCTOR
	public VMFrame(int physical_Memory, int TLB, int virtual_Memory){
		...
		physicalMemoryBit = physical_Memory;
		tLB = TLB;
		virtualMemory = virtual_Memory;
		
		physicalMemorySize=(int)Math.pow(2,physicalMemoryBit);
		tlBSize=(int)(Math.pow(2, tLB));
		virtualMemorySize=(int)(Math.pow(2, virtualMemory));
		...
		tProgress.setText("<html>System Configurations : "
						  + "TLB Size : "+ tLB + " | Virtual memory page : " + virtualMemorySize +" | " + " | Physical memory page : " + physicalMemorySize +" | "
						  + "<br> " + 4*physicalMemorySize +" Blocks in Main Memory, " + "8 words per block |  Page size = 32 words | 4 blocks per page."
		...
		//CREATE PANELS
		virtMemPanel = new VirtualMemoryPanel(virtualMemorySize);
		physMemPanel = new PhysicalMemoryPanel(8,physicalMemorySize);
		memPanel = new JPanel();
		tlbPanel = new TLBPanel(tLB);
		pageTablePanel = new PageTablePanel(virtualMemorySize);
		...
		JPanel tLBPanelALL = new JPanel();
		JPanel pageTablePanelALL = new JPanel();
		//ADD COMPONENTS TO THE PANELS	
		
		//PANEL WITH THE PHYSICAL AND VIRTUAL MEMORY
		memPanel.setLayout(new BorderLayout());
		physMemPanelScroll = new JScrollPane();
		physMemPanelScroll.getViewport().add(physMemPanel);
		physMemPanelScroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		physMemPanelScroll.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		physMemPanelScroll.getViewport().setScrollMode(JViewport.SIMPLE_SCROLL_MODE);
		memPanel.add(physMemPanelScroll, "Center");
		JScrollPane virtMemScroll = new JScrollPane();
		virtMemScroll.getViewport().add(virtMemPanel);
		virtMemScroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		virtMemScroll.getViewport().setScrollMode(JViewport.SIMPLE_SCROLL_MODE);
		virtMemScroll.getViewport().setOpaque(true);
		memPanel.add(virtMemScroll, "West");
		...
		pTables.setLayout(new GridLayout(2,1));
		tLBPanelALL.setLayout(new BorderLayout());
		
		JScrollPane tlbScroll = new JScrollPane();
		tlbScroll.getViewport().add(tlbPanel);
		tlbScroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		tlbScroll.getViewport().setScrollMode(JViewport.SIMPLE_SCROLL_MODE);
		tlbScroll.getViewport().setOpaque(true);
		tlbScroll.setPreferredSize(new Dimension(150,260));
		tLBPanelALL.add(tlbScroll,"North");
		tLBPanelALL.add(pTLBHitsMisses,"South");
		
		pageTablePanelALL.setLayout(new BorderLayout());
		JScrollPane pageTableScroll = new JScrollPane();
		pageTableScroll.getViewport().add(pageTablePanel);
		pageTableScroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		pageTableScroll.getViewport().setScrollMode(JViewport.SIMPLE_SCROLL_MODE);
		pageTableScroll.getViewport().setOpaque(true);
		pageTableScroll.setPreferredSize(new Dimension(200,260));
		pageTablePanelALL.add(pageTableScroll,"North");
		pageTablePanelALL.add(pPageHitsFaults,"South");
		
		pTables.add(tLBPanelALL);
		pTables.add(pageTablePanelALL);
		...
		*/
		addresses = CheckAddress.createAddresses(32*virtualMemorySize);
	...
	*/
	public void reStart(){
		...
		tProgress.setText("Please generate the Address Reference String."
						  +"<br>Then click on \"Next\" to continue.");
	...
	public void step(){
		...
		while (tempState <= moveStatus){
			...
			case 1: //IF A NEW CYCLE IS BEGINNING, OBTAIN NEXT ADDRESS REFERENCE
				...
				if (binAddressVM.length() <= virtualMemory+5){
					int zeroes = virtualMemory + 5 - binAddressVM.length();
					for (int i = 0; i <= zeroes; i++)
						binAddressVM = '0'+binAddressVM;
				}
				virtPage = binAddressVM.substring(0, binAddressVM.length() - 5);
				offset = binAddressVM.substring(binAddressVM.length() - 5);
			...
			case 2: //HIGHLIGHT THE BITS IN THE VIRTUAL ADDRESS 
				...
				if (tempState == moveStatus){
					tProgress.setText("<html>The hexadecimal address " + hexAddress 
									  ...
									  + "<br>Hence the bits in the Virtual Address are divided into "
									  + virtPage + " --> Page,  "+ offset + " --> Offset."
									  + "<br>The above field values are used to access the required virtual page from the virtual memory space.");
			...
			case 3: //SEARCH THE TLB FOR THE VIRTUAL PAGE NUMBER AND HIGHLIGHT THE RESULT, IF FOUND
				...
				//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
				if (tempState == moveStatus){
					tProgress.setText("<html>TLB is searched for  "+ virtPage 
									  + "<sub>2</sub> =  "+ intVirtPageDec + "<sub>10</sub>."  );
				...
				if (tlbEntry >= 0){
					...
					//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
					if (tempState == moveStatus){					
						tProgress.setText ( tProgress.getText() + " -  TLB HIT! A matching entry was found in the TLB for the virtual page number "+intVirtPageDec+"."
										 +"<br>The physical page frame number is retrieved from this entry. It is "+tlbPanel.physPageNum[tlbEntry]+"."
										 +"<br>Thus, the physical address bits are "+binAddressPM+"."
										 +"<br>The actual physical address is, therefore, "+physAddress);
					...
					//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
					if (tempState == moveStatus){
						tProgress.setText (tProgress.getText() + " - TLB MISS! A matching entry was not found for the virtual page number."
										 +"<br>Now the entry at row "+intVirtPageDec+" in the page table is looked up.");
					...
					if (pageTableEntry >= 0){//THERE WAS A PAGE HIT
						...
						//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
						if (tempState == moveStatus){
							tProgress.setText( tProgress.getText() + "<br> Page Hit! The virtual page number had a valid entry in the page table."
											 +"<br>The physical page number is "+pageTableEntry+"."
											 +"<br>Physical address bits are "+binAddressPM+"."
											 +"<br>Actual physical address : "+physAddress+"."
											 +"<br><br>Note that the TLB is now updated using the information found in the page table.");
						...
						//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
						if (tempState == moveStatus){
							tProgress.setText( tProgress.getText() + "<br>Page Fault! The page table entry for this virtual page number had a valid bit of 0."
											 +"<br>The page must now be brought into physical memory from the hard disk.");
			...
			case 4: //HIGHLIGHT THE PHYSICAL MEMORY FRAME WHERE THE PAGE EXISTS OR WILL EXIST
				...
				if (boolTLBHit){
					...
					//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
					if (tempState == moveStatus){
						tProgress.setText("<html>As we saw in the previous step, the page frame number was retrieved from the TLB."
										  +"<br>Highlighted is the page frame in physical memory with the data that we wanted."
										  +"<br>Since the offset retrieved from the virtual address was "+intOffsetDec+", the word at this offset from the first address in the frame is the required data word.");
				...
				//CASE 2: THERE WAS A PAGE TABLE HIT
					...
					//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
					if (tempState == moveStatus){
						tProgress.setText("<html>As we saw in the previous step, the page frame number was retrieved from the Page Table."
										  +"<br>Highlighted is the page frame in physical memory with the data that we wanted."
										  +"<br>Since the offset retrieved from the virtual address was "+intOffsetDec+", the word at this offset from the first address in the frame is the required data word.");
				...
				//THIS FRAME MAY BE AN AVAILABLE EMPTY FRAME IN MEMORY, OR THE LEAST RECENTLY USED ONE
					...
					if (!(emptyMemFrame == -1)){
						//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
						if (tempState == moveStatus){
							tProgress.setText("<html>Since the memory has empty space, the first available frame will be filled."
											  +"<br>Highlighted is the memory frame into which the page will be brought.");
					...
					//ELSE DISPLAY THE LRU MEMORY FRAME WHICH WILL BE REPLACED
						...
						//IF THIS IS THE CURRENT STATE, UPDATE THE PROGRESS FIELD
						if (tempState == moveStatus){
							tProgress.setText("<html>Since the memory is full, the least recently used memory frame will be replaced."
											  +"<br>Highlighted is the victim memory frame which will be replaced.");
			...
			case 6: //BRING THE PAGE INTO MEMORY AND HIGHLIGHT THE FRAME WITH THE DATA IN IT
				...
				if (boolTLBHit || boolPageHit){
					//IF THIS IS THE CURRENT STATE, UPDATE PROGRESS STATUS
					if (tempState == moveStatus){
						tProgress.setText("<html>As we saw earlier, the page already exists in frame "+frameNum+"."
										  +"<br>Highlighted in black is the required word at offset "+intOffsetDec+".");
					...
					//IF THIS IS THE CURRENT STATE, UPDATE PROGRESS STATUS
					if (tempState == moveStatus){
						tProgress.setText("<html>The page is brought in from disk to memory into frame "+frameNum+"."
										+"<br>Highlighted in black is the required word at offset "+intOffsetDec+".");
					...
					if (lruMemFrame >= 0){
						...
						//IF THIS IS THE CURRENT STATE, UPDATE PROGRESS STATUS
						if (tempState == moveStatus){
							tProgress.setText( tProgress.getText() + "<html><br>Note that, since the previous page "+pageNum+" that was in the frame was replaced,"
					...
					for (int j = 0; j < 32; j++)
							physMemPanel.stringWords[frameNum][j] = intVirtPageDec+"_"+j;
					...
					//IF THIS IS THE CURRENT STATE, UPDATE PROGRESS STATUS
					if (tempState == moveStatus){
						tProgress.setText( tProgress.getText() +  "<html><br>Note that the physical address was generated as the page was brought into memory."
										 +"<br>The physical page frame number is "+frameNum+"."
										 +"<br>Thus, the page bits in the physical address bits are "+physPage+"."
										 +"<br>The word bits are the same as in the virtual address, "+offset+"."
										 +"<br>The actual physical address in deacimal is, therefore, "+physAddress+".");
	...
	public void autoGenerateString(){
		...
		for (int i = 0; i < 10; i++){
			...
			random = (int)(Math.random() * 32* virtualMemorySize);
	...
	public void selfGenerateString(){
		...
		while ((option == 0) && index < 10){
			...
			if (option == 0){
				//THEN VALIDATE INPUT
				if ((CheckAddress.validateInput(textfield.getText(),(int)32* virtualMemorySize)) || (CheckAddress.validateInput(textfield.getText().toUpperCase(),(int)32* virtualMemorySize))){
			...
			//ELSE IF DONE BUTTON WAS CLICKED ON INPUT, VALIDATE INPUT
				...
				//	listData[index] = textfield.getText().toUpperCase();
					...
					tProgress.setText("<html>You have generated an Address Reference String of "+index+" address."
									  +"<br>Please click on \"Next\" to continue.</html>");
